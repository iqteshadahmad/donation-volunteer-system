        ...
        try:
            ...
            if user:
                if user.is_staff:
                    ...
                    messages.warning(request, "Invalid Admin Username")
...
class login_donor(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user:
                donor_user = Donor.objects.filter(user_id=user.id).first()
                if donor_user:
                    login(request, user)
                    messages.success(request, "Login Successfully")
            ...
            messages.warning(request, "Please fill all fields properly")
...
class login_volunteer(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user:
                vol_user = Volunteer.objects.filter(user_id=user.id).first()
                if vol_user:
                    login(request, user)
                    # messages.success(request, "Login Successfully")
...
def manage_donor(request):
    ...
    donor = Donor.objects.all().order_by('-regdate')
...
def accepted_volunteer(request):
    if not request.user.is_authenticated or not request.user.is_staff:
        return redirect('login_admin')
    volunteer = Volunteer.objects.filter(status__iexact='accepted')
    ...
    def post(self,request):
        ...
        try:
            if newpass == confirmpass:
                ...
                messages.warning(request,'New Password and Confirm New Password are different')
        except:
